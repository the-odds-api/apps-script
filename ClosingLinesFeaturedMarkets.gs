function getClosingLines() {
  /**
   * Query historical closing lines from the The Odds API and output the data to Google Sheets.
   * 
   * This script will only work with featured markets (h2h, spreads, totals). To query any market, see ClosingLinesAnyMarket.gs
   *
   ...
   * 
   * Note this code does not handle futures (outrights) markets.
   */

  const SPREADSHEET_URL = 'https://docs.google.com/spreadsheets/abc123/edit#gid=0' // Get this from your browser
  ...
  const SPORT_KEY = 'icehockey_nhl' // For a list of sport keys, see https://the-odds-api.com/sports-odds-data/sports-apis.html
  ...
  const BOOKMAKERS = 'betonlineag' // Optional - if specified, it overrides REGIONS. A list of comma separated bookmakers from any region. For example: draftkings,pinnacle See all bookmakers at https://the-odds-api.com/sports-odds-data/bookmaker-apis.html
  const ODDS_FORMAT = 'american' // Valid values are american and decimal.

  const FROM_DATE = '2023-12-31T00:00:00Z'
  const TO_DATE = '2024-01-01T00:00:00Z'
  ...
  for (const commenceTime in groupedEvents) {
    ...
    oddsResponse = fetchOdds(API_KEY, SPORT_KEY, REGIONS, BOOKMAKERS, MARKETS, ODDS_FORMAT, commenceTime, groupedEvents[commenceTime])
...
function fetchOdds(apiKey, sportKey, regions, bookmakers, markets, oddsFormat, timestamp, eventIds) {
  const bookmakersParam = bookmakers ? `bookmakers=${bookmakers}` : `regions=${regions}`
  const eventIdsParam = eventIds ? `&eventIds=${eventIds.join(',')}` : ''
  const url = `https://api.the-odds-api.com/v4/historical/sports/${sportKey}/odds?apiKey=${apiKey}&${bookmakersParam}&markets=${markets}&oddsFormat=${oddsFormat}&date=${timestamp}${eventIdsParam}`
